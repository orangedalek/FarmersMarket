...
var CreatePod = React.createClass({
  ...
  // will update the inputs from below into new initial state values empty
    ...
    db.Pod.save({title: this.state.title, icon: this.state.icon, description: this.state.description, keyword: this.state.keyword /*members: this.state.members*/});
    // specify /query/:id to PodcastSearch.js
    res.redirect('/query/:id', function(req, res) {
